...
check_success() {
    if [ $? -ne 0 ]; then
        print_red "Ошибка при выполнении команды!"
        exit 1
    fi
}
...
select opt in "${options[@]}"; do
    case $opt in
        "Полностью автоматическая установка")
            VNC_PASSWORD="172029"
            print_green "Выбранная версия: полностью автоматическая установка."
        ...
        "Ручная установка")
            ...
            print_green "Выбранная версия: ручная установка."
            echo "Выбранная версия: ручная установка." >> "$log_file"
            break
            ;;
        *) 
            print_red "Неверный выбор. Пожалуйста, попробуйте снова."
