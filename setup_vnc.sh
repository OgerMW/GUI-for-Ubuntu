...
log_message "Отключаем уведомления через конфигурацию APT"
{
    echo "Unattended-Upgrade::Automatic-Reboot \"false\";"
    echo "Unattended-Upgrade::Automatic-Reboot-WithUsers \"false\";"
    echo "APT::Periodic::Unattended-Upgrade \"0\";"
    echo "APT::Periodic::Unattended-Upgrade \"0\";"
} | sudo tee -a /etc/apt/apt.conf.d/50unattended-upgrades &>> "$LOG_FILE" &
show_loading $!
...
log_message "Создание systemd сервиса для VNC"
cat << EOF | sudo tee /etc/systemd/system/vncserver@.service &>> "$LOG_FILE" &
show_loading $!
